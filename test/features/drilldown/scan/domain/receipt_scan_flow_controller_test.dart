...
void main() {
  ...
  test('startScan runs the happy path through to awaitingConfirm', () async {
    ...
    expect(
      ...
      ReceiptScanPhase.idle,
    );

    final phases = <ReceiptScanPhase>[];
    container.listen(receiptScanFlowProvider, (_, next) {
  ...
  test('holdsImage clears once confirm persists and drops the photo',
    ...
    expect(container.read(receiptScanFlowProvider).holdsImage, isTrue);

    await controller.confirm();

    final state = container.read(receiptScanFlowProvider);
    expect(state.phase, ReceiptScanPhase.done);
    expect(state.holdsImage, isFalse);
  });
