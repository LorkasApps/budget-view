...
| receipt-scan.md | Drilldown | Ephemeral receipt capture: photo/PDF/gallery source picker, deskew, doc-hash check, OCR or PDF reading, parser seams, confirm → line-items + ImportedSource, printed total checksum banner |
| tagging.md | Tagging | TaggingRule entity, repository (upsert/hit-count, hard delete, remap), learn service + its three UI call sites, suggest service + shared suggestion sheet, TaggingRulesScreen (list/sort/remap/delete/stale-curation), providers |
| import.md | Transaction | PDF import layer: PdfParser interface, registry (IngGiroParser), ING layout parsing, import flow (preview, edit, persist via controller), import history screen + provider |
