...
- `deny` — secrets (`.env`, `secrets/`, `*.pem|jks|keystore|p12|pfx`, `key.properties`, SSH keys, `.npmrc`, `.netrc`, `credentials[.json]`), ballast (`build/`, `dist/`, `node_modules/`, `vendor/`, `coverage/`, `.next/`, `.venv/`, `__pycache__/`, `.dart_tool/`, `*.g.dart`, `*.lock`, `*.min.js|css`, `*.map`, `libisar.*`), or oversized. The secret and ballast lists are kept in step with `permissions.deny` in `.claude/settings.json` — the deny rules cover the file tools, this hook is what also sees `cat .npmrc`. `.claude/tmp/check.log` is deliberately **not** blocked — reading it after a failed gate is the documented workflow above; the size check governs it instead, so slice it with `offset`/`limit`
