...
## Acceptance Criteria
- [x] `SettingsScreen` gains one `Tagging-Regeln` row pushing the rule screen; nothing else on that screen changes
- [x] Rule list is flat, default `hitCount` DESC then `lastAssignedAt` DESC (the `findAll` order), switchable to
      `lastAssignedAt` and counterparty; comparators live in Dart, not in a query
- [x] Row shows `matchValueNorm`, the resolved category name, `hitCount`, `lastAssignedAt`
- [x] Category names resolve through `categoriesProvider` with `includeArchived: true`, so an archived category still
      renders its name next to the stale marker
- [x] A rule's category can be changed via the existing category picker, calling
      `TaggingRuleRepository.remap(uuid, categoryUuid)`; `hitCount` survives
- [x] Single-rule delete calls `TaggingRuleRepository.delete(uuid)` behind a confirm
- [x] Stale marker on every row whose `categoryUuid` resolves to nothing or to an `archived` category
- [x] One collective `Veraltete Regeln löschen` action, confirm dialog names the count, removes only stale rules
- [x] No way to create a rule by hand anywhere on the screen
- [x] Empty state when nothing has been learned yet
- [x] List is reactive (`StreamProvider` on the Isar collection watch, like `accountsProvider`): remap and delete are
      reflected without a manual refresh
- [x] Archiving a category leaves its rules in place — covered by a test
- [x] No new repository method is added. Needing one means the boundary against 013 is drawn wrong
- [x] `make check` green
...
- `test/features/tagging/domain/tagging_rule_category_archive_test.dart` — archiving a category leaves its rules in
  place with their `hitCount`, on a real Isar. The screen test only proves the marker renders; this proves the
  no-cascade guarantee the AC actually asks for
